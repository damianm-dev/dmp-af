...
- `tests`: installs a script to test dbt project ([tests tutorial](#_dmp-af_-tests))
...
`dmp-af` brings a lot of features into dbt project, and it's important to test them for correctness.

For now, available tests are:

1. Test that all models are presented in Airflow DAGs
2. Test that raw model config is correctly parsed by `dmp-af`
...
To run these tests, you need to install extra `tests` with _dmp-af_:

```bash
pip install dmp-af[tests]
```

This will make available a script `dmp-af-manifest-tests` to run the tests. Run to get mode information:
...
## minidbt
Run `pip install dmp-af[minidbt]`
