...
class DbtRunMacroOperation(DbtIntervalActionOperator):
    def __init__(self, dmp_af_config: Config, **kwargs):
        self.macro = self.macro_name

        super().__init__(
            task_id=self.af_task_name,
            dmp_af_config=dmp_af_config,
            retry_policy=dmp_af_config.retries_config.macros_retry_policy,
            **kwargs,
        )
        if self.macro_args:
...
class DbtPersistTableDocumentation(DbtRunMacroOperation):
    def __init__(self, model_name, maintenance_config: DmpAfMaintenanceConfig, **kwargs) -> None:
...
class DbtOptimizeTable(DbtRunMacroOperation):
    def __init__(
        self,
        model_name: str,
        maintenance_config: DmpAfMaintenanceConfig,
...
class DbtDeduplicateTable(DbtRunMacroOperation):
    def __init__(
        self, model_name: str, maintenance_config: DmpAfMaintenanceConfig, target_environment: str, **kwargs
    ) -> None:
        self.retries = 0
        self.model_name = model_name
        self.maintenance_config = maintenance_config
...
class DbtMaintenanceOperatorFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        target_env = (
            dmp_af_config.dbt_default_targets.default_maintenance_target
